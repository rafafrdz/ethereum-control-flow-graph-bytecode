...
sealed trait OpcodeId {
  val opcode: Int
  val name: String

  def bytes: String = hex(opcode)
...
object OpcodeId {
  def hex(hex: Int): String = f"0x${hex}%02X"

  def hex(hex: BigInt): String = f"0x${hex}%02X"
  ...
  /** Artificial Opcode Id */
  val RAW: OpcodeId = instance(0, "RAW")
}
