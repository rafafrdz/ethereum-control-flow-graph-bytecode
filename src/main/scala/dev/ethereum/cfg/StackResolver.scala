...
object StackResolver {
  ...
  def resolve[T <: Opcode](bcodes: List[BasicBlockOpcode[T]]): List[RelOpcode[T]] =
    from(Nil, bcodes).reverse

  @tailrec
  private def from[T <: Opcode](acc: List[RelOpcode[T]], rest: List[BasicBlockOpcode[T]]): List[RelOpcode[T]] = {
    rest match {
      ...
      case ::(b@BasicBlockOpcode(id, stack), next) if RelOpcode.iSymbolicJump(stack) => from(NonResolvedSymbolicRelOpcode(b) :: acc, next)
  ...
  @tailrec
  private def to[T <: Opcode](rest: List[BasicBlockOpcode[T]]): BasicBlockOpcode[T] =
    rest match {
      case ::(b@BasicBlockOpcode(_, stack), next) if RelOpcode.noSucc(stack) => to(next)
      case ::(b@BasicBlockOpcode(_, stack), next) => b
    }

  private def branch[T <: Opcode](rest: List[BasicBlockOpcode[T]]): (BasicBlockOpcode[T], BasicBlockOpcode[T], List[BasicBlockOpcode[T]]) = {
  ...
  private def falseTo[T <: Opcode](rest: List[BasicBlockOpcode[T]]): (BasicBlockOpcode[T], List[BasicBlockOpcode[T]]) = (rest.head, rest.tail)

  @tailrec
  private def trueTo[T <: Opcode](rest: List[BasicBlockOpcode[T]], diff: List[BasicBlockOpcode[T]]): (BasicBlockOpcode[T], List[BasicBlockOpcode[T]]) =
    rest match {
